...
# 全局变量初始化
# 封面抽出动画状态跟踪
album_hover_animations = {}  # 用于跟踪每个专辑的抽出动画状态
album_hover_animation_speed = 2.0  # 降低动画速度（原来是5.0）
...
def blur_in(speed: int = 1, max_blur_radius=30):  # 增大默认的最大模糊半径
    global background_blur_radius, blurred_background, dark_overlay_surface
    if background_blur_radius < max_blur_radius: # blur in animation
        background_blur_radius += speed
        if background_blur_radius > max_blur_radius:
            background_blur_radius = max_blur_radius

        # 更新模糊效果 (使用已存在的 overlay)
        blurred_background = background.copy()
        if dark_overlay_surface is None or dark_overlay_surface.get_size() != (WIDTH, HEIGHT):
             dark_overlay_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)

        alpha = min(background_blur_radius * 5, 150) # 降低alpha倍数，保持最大值150
        dark_overlay_surface.fill((0, 0, 0, alpha))
        blurred_background.blit(dark_overlay_surface, (0, 0))

def blur_out(speed: int = 1, min_blur_radius=0):
    global blurred_background, background_blur_radius, dark_overlay_surface
    if background_blur_radius > min_blur_radius: # blur animation
        background_blur_radius -= speed
        if background_blur_radius < min_blur_radius:
            background_blur_radius = min_blur_radius
        ...
        if background_blur_radius > 0:
            ...
            dark_overlay_surface.fill((0, 0, 0, alpha))
            blurred_background.blit(dark_overlay_surface, (0, 0))
        # else: # 如果 blur radius <= 0，理论上 blurred_background 就是 background
...
def set_music_position(mouse_x):
    """根据鼠标x坐标设置音乐播放位置"""
    global music_pos, music_start_time, target_progress_width, is_big_jump # Removed current_progress_width from globals as it's not reset here
    ...
    # 计算百分比位置
    position_percent = relative_x / total_progress_bar_width if total_progress_bar_width > 0 else 0 # Avoid division by zero

    # 计算新的位置（秒）
    new_pos = position_percent * id3[4] if id3 and id3[4] > 0 else 0 # Check for non-zero length
    ...
    # 设置进度条动画的目标宽度, 但不重置 current_progress_width
    target_progress_width = relative_x
    # current_progress_width = 0 # <-- 移除此行
...
if 1:
    ...
    # 2. 定义按钮的垂直位置 (不变)
    progress_bar_base_y = HEIGHT / 2 + 100 + 40 + 60
    time_text_height = small_font.get_height()
    button_top_margin = 25
    play_pause_btn_center_y = progress_bar_base_y + 10 + time_text_height + button_top_margin + play_pause_btn_radius
    play_pause_btn_y = play_pause_btn_center_y - play_pause_btn_radius
    prev_btn_y = play_pause_btn_center_y - prev_btn_radius
    next_btn_y = play_pause_btn_center_y - next_btn_radius
    loop_btn_y = play_pause_btn_center_y - loop_btn_radius

    # --- 修改 X 坐标计算逻辑 ---
    # 3. 定义上一首按钮的 X 坐标 (左边缘与进度条对齐)
    progress_bar_start_x = WIDTH / 25
    prev_btn_x = progress_bar_start_x # 左边缘对齐
    prev_btn_center_x = prev_btn_x + prev_btn_radius

    # 4. 计算播放/暂停按钮的 X 坐标 (基于上一首按钮)
    play_pause_btn_center_x = prev_btn_center_x + prev_btn_radius + button_spacing + play_pause_btn_radius
    play_pause_btn_x = play_pause_btn_center_x - play_pause_btn_radius

    # 5. 计算下一首和循环按钮的 X 坐标 (基于播放/暂停按钮)
    next_btn_center_x = play_pause_btn_center_x + play_pause_btn_radius + button_spacing + next_btn_radius
    next_btn_x = next_btn_center_x - next_btn_radius

    loop_btn_center_x = next_btn_center_x + next_btn_radius + button_spacing + loop_btn_radius
    loop_btn_x = loop_btn_center_x - loop_btn_radius
    # --- X 坐标计算结束 ---
    ...
    # 高斯模糊相关
    blur_animation_completed = False  # 标记高斯模糊飞入动画是否已完成
    ...
    # ... (其他全局变量)
    lyrics_animation_finished = False # 标记歌词入场动画是否结束
    lyrics_animation_finish_time = 0 # 记录歌词动画结束时间
    selected_music_path = None # 记录当前选中的音乐路径

    # 背景设置相关变量
    background_settings_active = False  # 是否显示背景设置界面
    background_options = []  # 存储所有背景选项
    background_hover_index = -1  # 当前鼠标悬浮的背景索引
    background_animation_active = False  # 背景切换动画是否激活
    background_animation_start_time = 0  # 背景切换动画开始时间
    background_animation_duration = 0.8  # 背景切换动画持续时间
    background_animation_target = None  # 目标背景图片
    background_animation_source = None  # 源背景图片
    background_button_hover = False  # 背景设置按钮是否被悬浮
    SETTINGS_BUTTON_SIZE = 30  # 设置按钮大小
    background_scroll_offset = 0  # 背景设置界面的滚动偏移
    background_target_scroll = 0  # 目标滚动位置
    background_scroll_velocity = 0  # 滚动速度
    background_scroll_active = False  # 是否正在平滑滚动
    BACKGROUND_SCROLL_DECELERATION = 0.85  # 滚动减速系数
...
'''Music Player by Doushabao_233 and KMNO4
now:{fps} fps (setfps: 120), true: {actual_fps}
...
background alpha: {info_bg_alpha}
blur val: {bg_blur_var}
music long: {total_prog_bar_width}
music data: {music_meta}
'''.format(
        ...
        if background_animation_active:
            update_background_animation(screen)
        else:
            # 正常绘制背景
        ...
        if music_busy or album_animation_active or end_animation_active or song_transition_active:
            ...
            # 否则绘制音乐播放界面
                ...
                if target_progress_width > 0:
                    # 拖动后的动画
                    delta = target_progress_width - current_progress_width
                    if abs(delta) < 1: # 如果非常接近目标
                        current_progress_width = target_progress_width # 直接设置为目标值
                        target_progress_width = 0 # 重置目标值，结束动画状态，恢复正常更新
                    elif abs(delta) < 10:
                        # 接近目标时减速
                        current_progress_width += delta * 0.1
                    else:
                        # 快速接近目标
                        current_progress_width += delta * 0.3
                else:
                    # 正常播放时，进度条宽度与音乐播放位置同步
                    current_progress_width = total_progress_bar_width * (music_pos / id3[4]) if id3 and id3[4] > 0 else 0 # 添加 id3[4] > 0 检查

                # 确保 current_progress_width 不会超出范围
                current_progress_width = min(current_progress_width, total_progress_bar_width)
                current_progress_width = max(0, current_progress_width) # 确保非负
                # --- 进度条宽度更新逻辑结束 ---
                ...
                # 绘制信息背景 (带透明度)
                screen.blit(info_background, (0, 0)) # info_background 具有 fade_in 设置的透明度
                ...
                # --- 在 progress_bar_surface 上绘制进度条 ---
                # 绘制进度条背景
                pygame.draw.rect(progress_bar_surface, (150, 150, 150),
                               pygame.Rect(WIDTH / 25, progress_bar_y,
                ...
                if is_mouse_over_progress_bar(pygame.mouse.get_pos()):
                    ...
                    indicator_rect = pygame.Rect(indicator_x - 3, progress_bar_y - 5, 6, 20)
                    pygame.draw.rect(progress_bar_surface, (255, 255, 255), indicator_rect, border_radius=2)
                # --- 进度条绘制结束 ---

                # --- 将文本绘制到 info_background (先清空) ---
                info_background.fill((0, 0, 0, 0)) # 清空 info_background 为透明
                ...
                # --- 将进度条直接绘制到屏幕 screen ---
                screen.blit(progress_bar_surface, (0, 0)) # 绘制进度条到主屏幕
        ...
        # 如果背景设置界面激活，绘制设置界面
        if background_settings_active:
            draw_background_settings(screen)
...
main_ui_animation_duration = 1.5  # 主界面UI动画持续时间(秒) (原来是0.8)
...
dark_overlay_surface = None # 用于模糊效果的叠加层
...
def draw_album_list(screen):
    ...
    for i in range(start_index, min(len(music_library), start_index + max_visible_albums + 2)):  # 多渲染两个以避免滚动空隙
        ...
        if album_cover:
            ...
            # 创建一个带圆角的遮罩表面
            rounded_mask = pygame.Surface((int(album_width), int(album_height)), pygame.SRCALPHA)
            pygame.draw.rect(rounded_mask, (255, 255, 255), rounded_mask.get_rect(), border_radius=10) # 使用白色绘制圆角矩形

            # 将调整大小后的封面应用遮罩
            rounded_cover_surf = resized_cover.copy() # 复制一份以应用遮罩
            rounded_cover_surf.blit(rounded_mask, (0, 0), special_flags=pygame.BLEND_RGBA_MULT) # 应用遮罩

            # 根据选中和悬停状态添加效果 (绘制边框)
            if i == selected_album_index:
                # 选中的专辑显示边框和阴影效果
                shadow_rect = pygame.Rect(album_x - 5, album_y - 5, album_width + 10, album_height + 10)
                pygame.draw.rect(screen, (50, 50, 50, 128), shadow_rect, border_radius=15) # 阴影
                border_rect = pygame.Rect(album_x - 3, album_y - 3, album_width + 6, album_height + 6)
                pygame.draw.rect(screen, (230, 230, 230), border_rect, border_radius=12) # 亮色边框
            elif i == hover_album_index:
                # 悬停的专辑显示轻微边框
                border_rect = pygame.Rect(album_x - 2, album_y - 2, album_width + 4, album_height + 4)
                pygame.draw.rect(screen, (180, 180, 180), border_rect, border_radius=10) # 灰色边框

            # 绘制带圆角的专辑封面 (替换原来的 screen.blit(resized_cover, ...))
            screen.blit(rounded_cover_surf, (album_x, album_y))
...
def play_selected_album():
    ...
    if selected_album_index >= 0 and selected_album_index < len(music_library):
        ...
        album_animation_music_started = False
        ...
        try:
            ...
            print(f"音乐已加载: {selected_music_path}")
        except Exception as e:
            print(f"加载音乐失败: {selected_music_path}, 错误: {e}")
        ...
        # 动画结束后会开始UI元素动画 (这部分逻辑移到主循环中，在延迟结束后触发)
        # if not album_animation_active:
        #     start_ui_animation()

def draw_album_animation(screen):
    """绘制专辑封面动画"""
    global album_animation_active, album_animation_start_time, album_animation_surface, music_busy, music_start_time, animation_start_time, is_playing, music_paused, album_animation_phase, album_animation_music_started, parsed_lyrics, lyrics_surfaces, lyrics_positions, lyrics_need_update, total_progress_bar_width, current_progress_width, progress_bar_surface, info_background, lyrics_animation_finished, lyrics_animation_finish_time, full_lyrics_surface, background_blur_radius, blurred_background, dark_overlay_surface, blur_animation_completed # 添加blur_animation_completed标志
    ...
    # 处理高斯模糊从下向上飞入的效果 - 只在第一次进入播放界面时执行
    if not blur_animation_completed:
        # 在动画开始的2/3时间内完成模糊效果
        blur_animation_progress = min(1.0, elapsed / 0.6)  # 0.6秒内完成模糊效果
        
        # 使用缓动函数使模糊效果更自然
        blur_eased_progress = ease_out_cubic(blur_animation_progress)
        
        # 设置目标模糊半径 - 增大模糊强度
        target_blur_radius = 30  # 增大模糊强度到最大
        
        # 更新模糊半径
        background_blur_radius = target_blur_radius * blur_eased_progress
        
        # 模糊效果的Y偏移，从底部向上飞入
        blur_y_offset = HEIGHT * (1.0 - blur_eased_progress)
        
        # 更新模糊背景
        if blur_animation_progress > 0:
            # 更新模糊效果
            blurred_background = background.copy()
            if dark_overlay_surface is None or dark_overlay_surface.get_size() != (WIDTH, HEIGHT):
                dark_overlay_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
                
            # 计算当前alpha值 - 保持较低的alpha以避免增加对比度
            alpha = min(background_blur_radius * 5, 150)  # 降低alpha倍数，保持最大值150
            dark_overlay_surface.fill((0, 0, 0, alpha))
            blurred_background.blit(dark_overlay_surface, (0, 0))
            
            # 绘制模糊背景，考虑Y轴偏移
            temp_blur_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            # 从底部裁剪出一部分模糊背景
            visible_height = HEIGHT - blur_y_offset
            if visible_height > 0:
                # 计算裁剪区域
                clip_rect = pygame.Rect(0, 0, WIDTH, visible_height)
                # 将模糊背景绘制到临时表面
                temp_blur_surface.blit(blurred_background, (0, blur_y_offset), clip_rect)
                # 绘制正常背景（底部可见部分）
                screen.blit(background, (0, 0))
                # 在上面绘制飞入的模糊背景部分
                screen.blit(temp_blur_surface, (0, 0))
            else:
                # 如果模糊效果尚未可见，只绘制普通背景
                screen.blit(background, (0, 0))
            
            # 检查是否完成模糊动画
            if blur_animation_progress >= 1.0:
                blur_animation_completed = True
        else:
            # 动画开始前只绘制普通背景
            screen.blit(background, (0, 0))
    else:
    ...
    if album_animation_phase == 0:
        ...
        if album_animation_surface:
            try:
                ...
                # 创建一个带圆角的遮罩表面
                rounded_mask = pygame.Surface((int(current_width), int(current_height)), pygame.SRCALPHA)
                pygame.draw.rect(rounded_mask, (255, 255, 255), rounded_mask.get_rect(), border_radius=10) # 使用白色绘制圆角矩形
                
                # 将调整大小后的封面应用遮罩
                rounded_cover = scaled_cover.copy() # 复制一份以应用遮罩
                rounded_cover.blit(rounded_mask, (0, 0), special_flags=pygame.BLEND_RGBA_MULT) # 应用遮罩

                # 绘制到屏幕
                screen.blit(rounded_cover, (current_x, current_y))
    ...
    # 阶段2：歌词从右向左移入阶段
        ...
        if album_animation_surface:
            try:
                ...
                # 创建一个带圆角的遮罩表面
                rounded_mask = pygame.Surface((int(final_width), int(final_height)), pygame.SRCALPHA)
                pygame.draw.rect(rounded_mask, (255, 255, 255), rounded_mask.get_rect(), border_radius=10) # 使用白色绘制圆角矩形
                
                # 将调整大小后的封面应用遮罩
                rounded_cover = final_cover.copy() # 复制一份以应用遮罩
                rounded_cover.blit(rounded_mask, (0, 0), special_flags=pygame.BLEND_RGBA_MULT) # 应用遮罩
                
                screen.blit(rounded_cover, (final_x, final_y))

                # 绘制歌曲信息和进度条动画 (这部分逻辑保持，但注意文本现在应直接绘制到屏幕)
                if id3:
                    # 设置进度条的目标宽度 - 逐渐扩展
                ...
                # 动画完成判断
                if progress >= 1.0 and not lyrics_animation_finished: # 仅在首次完成时设置
                    # 检查所有歌词是否已完全到位
                    if all_lyrics_in_position:
                        print("歌词动画视觉上完成，准备延迟启动音乐。")
                        # 确保所有歌词精确到达最终位置
                        if parsed_lyrics and lyrics_positions:
                            for pos_info in lyrics_positions:
                                pos_info['current_x'] = pos_info['target_x']
                        lyrics_need_update = True # 最后更新一次确保位置精确

                        # 设置完成标志和时间，但不结束 album_animation_active
                        lyrics_animation_finished = True
                        lyrics_animation_finish_time = current_time

                # 如果动画已完成（包括延迟阶段），继续绘制最后一帧
                if lyrics_animation_finished:
                    # 确保绘制的是最终状态
                    # 绘制最终大小的封面
                    final_width = WIDTH / 3.4; final_height = WIDTH / 3.4; final_x = WIDTH / 25; final_y = WIDTH / 25
                    final_cover = pygame.transform.smoothscale(album_animation_surface, (int(final_width), int(final_height)))
                    screen.blit(final_cover, (final_x, final_y))
                    # 绘制最终进度条 (宽度为0)
                    progress_bar_surface.fill((0,0,0,0))
                    progress_bar_y = HEIGHT / 2 + 100 + 40 + 60
                    pygame.draw.rect(progress_bar_surface, (150, 150, 150), pygame.Rect(WIDTH / 25, progress_bar_y, total_progress_bar_width, 10), border_radius=10) # total_progress_bar_width 此时应接近目标值，但前景为0
                    screen.blit(progress_bar_surface, (0,0))
                    # 绘制最终歌词位置
                    if parsed_lyrics and lyrics_surfaces:
                        lyrics_area_width = WIDTH - (WIDTH / 25 + final_width + 60)
                        lyrics_display = pygame.Surface((lyrics_area_width, HEIGHT), pygame.SRCALPHA); lyrics_display.fill((0,0,0,0))
                        if full_lyrics_surface is None or lyrics_need_update: # 确保最后渲染一次精确位置
                            full_lyrics_surface = pygame.Surface((lyrics_area_width, HEIGHT), pygame.SRCALPHA); full_lyrics_surface.fill((0, 0, 0, 0))
                            line_height = 80
                            for i, surface in enumerate(lyrics_surfaces):
                                if i < len(lyrics_positions):
                                    y_pos = lyrics_positions[i]['target_y'] # 使用目标 Y
                                    x_offset = 0 # 已经到达目标 X
                                    if 80 <= y_pos <= HEIGHT - 30:
                                        lyrics_display.blit(surface, (x_offset, y_pos))
                            lyrics_need_update = False
...
def draw_end_animation(screen):
    """绘制音乐结束时的黑屏过渡动画"""
    global end_animation_active, end_animation_start_time, end_animation_black_screen_alpha, music_busy, music_metadata
...
def draw_song_transition(screen):
    ...
    transition_surface.fill((0, 0, 0, 0))  # 透明背景
    ...
    new_text_start_y = HEIGHT + 100  # 从屏幕下方开始
    new_text_end_y = HEIGHT / 2 + 100
    new_text_y = new_text_start_y + (new_text_end_y - new_text_start_y) * eased_progress
    
    # 绘制旧封面 (向左移动)
    transition_surface.blit(song_transition_old_cover, (old_cover_x, WIDTH / 25))
    
    # 绘制新封面 (从右侧移入)
    transition_surface.blit(song_transition_new_cover, (new_cover_x, WIDTH / 25))
    
    # 绘制旧文本 (向上移动)
    # 创建独立的文本表面以保持透明度
    old_title_surface = pygame.Surface((song_transition_old_title.get_width(), song_transition_old_title.get_height()), pygame.SRCALPHA)
    old_title_surface.blit(song_transition_old_title, (0, 0))
    
    old_artist_surface = pygame.Surface((song_transition_old_artist.get_width(), song_transition_old_artist.get_height()), pygame.SRCALPHA)
    old_artist_surface.blit(song_transition_old_artist, (0, 0))
    
    # 根据进度调整透明度
    fade_alpha = int(255 * (1 - eased_progress))
    old_title_surface.set_alpha(fade_alpha)
    old_artist_surface.set_alpha(fade_alpha)
    
    transition_surface.blit(old_title_surface, (WIDTH / 25, old_text_y))
    transition_surface.blit(old_artist_surface, (WIDTH / 25, old_text_y + 40))
    
    # 绘制新文本 (从下方移入)
    # 创建独立的文本表面以保持透明度
    new_title_surface = pygame.Surface((song_transition_new_title.get_width(), song_transition_new_title.get_height()), pygame.SRCALPHA)
    new_title_surface.blit(song_transition_new_title, (0, 0))
    
    new_artist_surface = pygame.Surface((song_transition_new_artist.get_width(), song_transition_new_artist.get_height()), pygame.SRCALPHA)
    new_artist_surface.blit(song_transition_new_artist, (0, 0))
    
    # 根据进度调整透明度
    fade_in_alpha = int(255 * eased_progress)
    new_title_surface.set_alpha(fade_in_alpha)
    new_artist_surface.set_alpha(fade_in_alpha)
    
    transition_surface.blit(new_title_surface, (WIDTH / 25, new_text_y))
    transition_surface.blit(new_artist_surface, (WIDTH / 25, new_text_y + 40))
...
def update_main_ui_animation():
    """更新主界面UI动画"""
    global main_ui_album_list_y, main_ui_animation_active
    
    if not main_ui_animation_active:
        return False
    
    # 计算动画进度
    elapsed = time.time() - main_ui_animation_start_time
    
    # 更新UI元素Y位置 - 使用缓动函数
    progress = min(1.0, elapsed / main_ui_animation_duration)
    eased_progress = ease_in_out_cubic(progress)
    main_ui_album_list_y = HEIGHT + 250 - (HEIGHT + 250 - main_ui_album_list_target_y) * eased_progress  # 增加起始位置的距离
    
    # 更新每个专辑的动画
    animation_still_active = False
    for i, album_info in enumerate(main_ui_albums_animated):
        # 考虑每个专辑的延迟
        album_elapsed = elapsed - album_info['delay']
        if album_elapsed <= 0:
            animation_still_active = True
            continue
        
        album_progress = min(1.0, album_elapsed / (main_ui_animation_duration * 1.2))  # 增加单个专辑动画时间
        album_eased_progress = ease_in_out_cubic(album_progress)
        
        # 只有在动画中才更新位置
        if album_progress < 1.0:
            animation_still_active = True
            album_info['current_y'] = album_info['start_y'] - (album_info['start_y'] - album_info['target_y']) * album_eased_progress
        else:
            album_info['current_y'] = album_info['target_y']
    
    # 检查整体动画是否结束
    if not animation_still_active and elapsed >= main_ui_animation_duration + (len(main_ui_albums_animated) - 1) * 0.25:  # 增加延迟检查时间
...
def is_mouse_over_horizontal_album(mouse_pos, album_index):
    ...
    # 获取当前封面的显示高度比例
    current_visible_ratio = 0.33  # 默认显示1/3
    if album_index in album_hover_animations:
        current_visible_ratio = album_hover_animations[album_index]['current_ratio']
    
    # 计算显示的高度部分
    visible_height = album_height * current_visible_ratio
    
    # 计算专辑Y位置 - 让显示部分紧贴窗口底部
    if main_ui_animation_active and album_index < len(main_ui_albums_animated):
        base_y = main_ui_albums_animated[album_index]['current_y']
    else:
        base_y = main_ui_album_list_y
    
    album_y = base_y + album_height - visible_height
    
    # 检查鼠标是否在专辑可见区域内
    return (album_x <= mouse_pos[0] <= album_x + album_width and 
            album_y <= mouse_pos[1] <= album_y + visible_height)
...
def draw_horizontal_album_list(screen):
    ...
    # 更新所有专辑的抽出动画
    for album_idx in list(album_hover_animations.keys()):
        anim_info = album_hover_animations[album_idx]
        target_ratio = anim_info['target_ratio']
        current_ratio = anim_info['current_ratio']
        
        # 如果接近目标值，则移除动画状态
        if abs(current_ratio - target_ratio) < 0.01:
            if target_ratio == 0.33:  # 如果已经还原到默认状态，可以从动画字典中移除
                album_hover_animations.pop(album_idx, None)
            else:
                album_hover_animations[album_idx]['current_ratio'] = target_ratio
        else:
            # 计算动画进度
            if 'progress' not in anim_info:
                anim_info['progress'] = 0.0
                anim_info['start_ratio'] = current_ratio
            
            # 更新进度
            if target_ratio > anim_info['start_ratio']:  # 展开动画
                anim_info['progress'] += album_hover_animation_speed * dt
                if anim_info['progress'] >= 1.0:
                    anim_info['progress'] = 1.0
                    album_hover_animations[album_idx]['current_ratio'] = target_ratio
                else:
                    # 使用缓动函数
                    t = ease_out_quad(anim_info['progress'])
                    album_hover_animations[album_idx]['current_ratio'] = anim_info['start_ratio'] + (target_ratio - anim_info['start_ratio']) * t
            else:  # 收回动画
                anim_info['progress'] += album_hover_animation_speed * dt
                if anim_info['progress'] >= 1.0:
                    anim_info['progress'] = 1.0
    ...
    for i in range(start_index, min(len(music_library), start_index + max_visible_albums + 2)):  # 多渲染两个以避免滚动空隙
        ...
        # 如果鼠标悬停或者专辑被选中，显示完整封面
        if i == hover_album_index or i == selected_album_index:
            target_ratio = 1.0
            if i not in album_hover_animations:
                album_hover_animations[i] = {
                    'current_ratio': 0.33, 
                ...
                # 如果目标已经是1.0但当前方向是收回，需要重置动画
                if album_hover_animations[i]['target_ratio'] != target_ratio:
                    album_hover_animations[i]['target_ratio'] = target_ratio
                    album_hover_animations[i]['progress'] = 0.0
                    album_hover_animations[i]['start_ratio'] = album_hover_animations[i]['current_ratio']
            
            current_visible_ratio = album_hover_animations[i]['current_ratio']
        else:
            # 不是悬停状态，目标是只显示1/3
            target_ratio = 0.33
            if i in album_hover_animations:
                # 如果目标已经是0.33但当前方向是展开，需要重置动画
                if album_hover_animations[i]['target_ratio'] != target_ratio:
                    album_hover_animations[i]['target_ratio'] = target_ratio
                    album_hover_animations[i]['progress'] = 0.0
                    album_hover_animations[i]['start_ratio'] = album_hover_animations[i]['current_ratio']
        ...
        if main_ui_animation_active and i < len(main_ui_albums_animated):
            base_y = main_ui_albums_animated[i]['current_y']
        else:
            base_y = main_ui_album_list_y  # 使用默认位置
        ...
        if album_cover:
            ...
            # 制作裁剪后的封面图像 - 显示顶部的部分
            clipped_cover = pygame.Surface((int(album_width), int(visible_height)), pygame.SRCALPHA)
            clipped_cover.blit(rounded_cover_surf, (0, 0), 
                             (0, 0, album_width, visible_height))
            
            # 根据选中和悬停状态添加效果 (绘制边框)
            if i == selected_album_index:
                # 选中的专辑显示边框和阴影效果
                shadow_rect = pygame.Rect(album_x - 5, album_y - 5, album_width + 10, visible_height + 5)
                pygame.draw.rect(screen, (50, 50, 50, 128), shadow_rect, border_radius=10) # 阴影
                border_rect = pygame.Rect(album_x - 3, album_y - 3, album_width + 6, visible_height + 3)
                pygame.draw.rect(screen, (230, 230, 230), border_rect, border_radius=10) # 亮色边框
            elif i == hover_album_index:
                # 悬停的专辑显示轻微边框
                border_rect = pygame.Rect(album_x - 2, album_y - 2, album_width + 4, visible_height + 2)
                pygame.draw.rect(screen, (180, 180, 180), border_rect, border_radius=10) # 灰色边框
            
            # 绘制裁剪后的专辑封面
            screen.blit(clipped_cover, (album_x, album_y))
            
            # 添加专辑信息标签(在封面下方) - 只在完全显示或接近完全显示时显示
            if current_visible_ratio > 0.8 and album_info.get('title'):
                # 裁剪过长的标题
                truncated_title = album_info['title']
                ...
                screen.blit(title_bg, (album_x + album_width/2 - title_text.get_width()/2 - 5, album_y + visible_height - 30))
                screen.blit(title_text, (album_x + album_width/2 - title_text.get_width()/2, album_y + visible_height - 27))
...
def start_main_ui_animation():
    ...
    main_ui_album_list_y = HEIGHT + 250  # 增加起始位置的距离（原来是150）
    
    # 初始化每个专辑的动画状态
    main_ui_albums_animated = []
    for i in range(len(music_library)):
        main_ui_albums_animated.append({
            'start_y': HEIGHT + 250,  # 增加起始位置的距离（原来是150）
            'target_y': main_ui_album_list_target_y,  # 目标位置
            'current_y': HEIGHT + 250,  # 增加起始位置的距离（原来是150）
            'delay': i * 0.25  # 增加每个专辑的动画延迟（原来是0.15）
        })
...
def is_mouse_over_settings_btn(mouse_pos):
    """检查鼠标是否在背景设置按钮上"""
    btn_x = WIDTH - SETTINGS_BUTTON_SIZE - 20  # 右上角位置
    btn_y = 20  # 顶部边距
    return (btn_x <= mouse_pos[0] <= btn_x + SETTINGS_BUTTON_SIZE and 
            btn_y <= mouse_pos[1] <= btn_y + SETTINGS_BUTTON_SIZE)

def draw_settings_button(screen):
    """绘制背景设置按钮"""
    global background_button_hover
    
    # 检查鼠标是否悬浮在按钮上
    mouse_pos = pygame.mouse.get_pos()
    background_button_hover = is_mouse_over_settings_btn(mouse_pos)
    
    # 按钮位置
    btn_x = WIDTH - SETTINGS_BUTTON_SIZE - 20
    btn_y = 20
    
    # 按钮颜色（悬浮时稍暗）
    btn_color = (245, 245, 245) if background_button_hover else (255, 255, 255)
    
    # 绘制齿轮图标
    center_x = btn_x + SETTINGS_BUTTON_SIZE // 2
    center_y = btn_y + SETTINGS_BUTTON_SIZE // 2
    outer_radius = SETTINGS_BUTTON_SIZE // 2 - 2
    inner_radius = outer_radius - 6
    tooth_length = 4
    
    # 绘制内圆
    pygame.draw.circle(screen, btn_color, (center_x, center_y), inner_radius)
    
    # 绘制齿轮齿
    for i in range(8):  # 8个齿
        angle = i * (math.pi / 4)
        # 外齿
        outer_x = center_x + (outer_radius + tooth_length) * math.cos(angle)
        outer_y = center_y + (outer_radius + tooth_length) * math.sin(angle)
        # 内齿
        inner_x = center_x + outer_radius * math.cos(angle)
        inner_y = center_y + outer_radius * math.sin(angle)
        # 绘制齿
        pygame.draw.line(screen, btn_color, (inner_x, inner_y), (outer_x, outer_y), 3)
...
def load_background_options():
    ...
    # 遍历背景文件夹
    for theme_dir in os.listdir(path):
        theme_path = os.path.join(path, theme_dir)
        if os.path.isdir(theme_path):
            # 加载每个主题的三个时段背景
            day_path = os.path.join(theme_path, 'day.png')
            noon_path = os.path.join(theme_path, 'noon.png')
            night_path = os.path.join(theme_path, 'night.png')
            ...
            if os.path.exists(day_path):
                backgrounds.append({
                    ...
                    'time': 'day',
                    'surface': None,  # 延迟加载
                    'hover_scale': 1.0,  # 悬浮时的缩放比例
                    'target_scale': 1.0  # 目标缩放比例
                })
...
def draw_background_settings(screen):
    """绘制背景设置界面"""
    global background_hover_index, background_animation_active, background_animation_start_time
    global background_animation_target, background_animation_source, background_scroll_offset
    ...
    # 标题
    title_text = bold_font.render("背景设置", True, (255, 255, 255))
    screen.blit(title_text, (50, 50))
    
    # 创建一个裁剪区域的表面
    settings_surface = pygame.Surface((WIDTH, HEIGHT - 100), pygame.SRCALPHA)
    settings_surface.fill((0, 0, 0, 0))
    
    # 计算网格布局
    thumb_size = WIDTH // 5  # 缩略图大小
    margin = 30  # 边距
    cols = 3  # 每行显示数量
    start_x = (WIDTH - (thumb_size * cols + margin * (cols - 1))) // 2
    start_y = 20  # 相对于裁剪区域的起始Y坐标
    
    # 检查鼠标位置 - 转换为相对于裁剪区域的坐标
    mouse_pos = pygame.mouse.get_pos()
    # 修正：鼠标Y坐标只需要减去顶部偏移，不需要加上滚动偏移
    # 因为项目位置已经包含了滚动偏移的计算
    relative_mouse_y = mouse_pos[1] - 100
    hover_found = False
    ...
    for i, bg in enumerate(background_options):
        ...
        # 计算项目的实际Y坐标（已经考虑了滚动偏移）
        actual_y = start_y + row * (thumb_size + margin) - background_scroll_offset
        
        # 如果在可视区域外，跳过渲染
        if actual_y + thumb_size < 0 or actual_y > HEIGHT - 100:
            continue
        
        # 延迟加载缩略图
        if bg['surface'] is None:
            try:
                full_img = pygame.image.load(bg['path']).convert()
                bg['surface'] = pygame.transform.smoothscale(full_img, (thumb_size, thumb_size))
            except:
                continue
        
        # 检查鼠标悬浮 - 使用正确的相对坐标比较
        is_hovered = (x <= mouse_pos[0] <= x + thumb_size and 
                     actual_y <= relative_mouse_y <= actual_y + thumb_size)
        
        if is_hovered:
            hover_found = True
            background_hover_index = i
            # 使用快入缓出的动画更新悬浮缩放
            if bg['hover_scale'] < 1.1:
                bg['target_scale'] = 1.1
            elif bg['hover_scale'] > 1.1:
                bg['hover_scale'] = 1.1
        else:
            # 恢复正常大小
            if bg['hover_scale'] > 1.0:
                bg['target_scale'] = 1.0
        
        # 更新缩放动画
        if bg['hover_scale'] != bg['target_scale']:
            # 快入缓出动画
            delta = bg['target_scale'] - bg['hover_scale']
            bg['hover_scale'] += delta * 0.2
        
        # 计算缩放后的尺寸和位置
        scaled_size = int(thumb_size * bg['hover_scale'])
        scaled_x = x - (scaled_size - thumb_size) // 2
        scaled_y = actual_y - (scaled_size - thumb_size) // 2
        
        # 绘制缩略图
        scaled_surface = pygame.transform.smoothscale(bg['surface'], (scaled_size, scaled_size))
        settings_surface.blit(scaled_surface, (scaled_x, scaled_y))
        
        # 绘制主题名称和时段
        theme_text = small_font.render(f"{bg['theme']} - {bg['time']}", True, (200, 200, 200))
        text_x = x + (thumb_size - theme_text.get_width()) // 2
        text_y = actual_y + thumb_size + 5
        settings_surface.blit(theme_text, (text_x, text_y))
    
    # 将裁剪后的设置界面绘制到屏幕上
    screen.blit(settings_surface, (0, 100))
    
    # 绘制滚动条
    if len(background_options) > 9:  # 如果背景数量超过一页
        # 计算滚动条位置和大小
        scroll_height = HEIGHT - 200  # 可滚动区域高度
        content_height = ((len(background_options) - 1) // 3 + 1) * (thumb_size + margin)  # 内容总高度
        if content_height > 0:  # 防止除零错误
            thumb_height = max(50, scroll_height * (scroll_height / content_height))  # 滚动条滑块高度
            thumb_pos = 100 + (scroll_height - thumb_height) * (background_scroll_offset / max(1, content_height - scroll_height))
            
            # 绘制滚动条背景
            pygame.draw.rect(screen, (100, 100, 100), 
                            pygame.Rect(WIDTH - 20, 100, 5, scroll_height),
                            border_radius=3)
            
            # 绘制滚动条滑块
            pygame.draw.rect(screen, (180, 180, 180),
                            pygame.Rect(WIDTH - 20, thumb_pos, 5, thumb_height),
                            border_radius=3)
    
    if not hover_found:
        background_hover_index = -1

def start_background_animation(target_path):
    """开始背景切换动画"""
    global background_animation_active, background_animation_start_time
    global background_animation_target, background_animation_source, background_settings_active
    
    # 加载新背景
    try:
        new_background = pygame.image.load(target_path).convert()
        new_background = pygame.transform.smoothscale(new_background, (WIDTH, HEIGHT))
        
        # 保存当前背景作为源
        background_animation_source = background.copy()
        background_animation_target = new_background
        
        # 开始动画
        background_animation_active = True
        background_animation_start_time = time.time()
        background_settings_active = False  # 关闭设置界面
        
    except Exception as e:
        print(f"加载背景图片失败: {e}")

def update_background_animation(screen):
    """更新背景切换动画"""
    global background_animation_active, background, background_animation_source
    global background_animation_target
    
    if not background_animation_active:
        return
    
    current_time = time.time()
    elapsed = current_time - background_animation_start_time
    progress = min(1.0, elapsed / background_animation_duration)
    ...
    if progress < 1.0:
        ...
        temp_surface.blit(background_animation_source, (0, 0))
        # 使用alpha混合创建过渡效果
        background_animation_target.set_alpha(int(255 * eased_progress))
        temp_surface.blit(background_animation_target, (0, 0))
        screen.blit(temp_surface, (0, 0))
    else:
        # 动画结束，更新背景
        background = background_animation_target.copy()
        background_animation_active = False
        background_animation_source = None
        background_animation_target = None

def update_background_scroll():
    """更新背景设置界面的滚动位置"""
    global background_scroll_offset, background_target_scroll, background_scroll_velocity, background_scroll_active
    
    # 计算内容高度和最大滚动范围
    thumb_size = WIDTH // 5
    margin = 30
    content_height = ((len(background_options) - 1) // 3 + 1) * (thumb_size + margin)
    max_scroll = max(0, content_height - (HEIGHT - 200))
    
    # 如果内容不需要滚动，直接返回
    if max_scroll <= 0:
        background_scroll_offset = 0
        background_scroll_velocity = 0
        return
        
    if background_scroll_active:
        # 应用滚动速度
        if abs(background_scroll_velocity) > 0:
            background_scroll_offset += background_scroll_velocity
            # 更柔和的减速效果
            background_scroll_velocity *= 0.92
            
            # 当速度很小时停止滚动，但保持当前位置
            if abs(background_scroll_velocity) < 0.1:
                background_scroll_velocity = 0
                # 注意：不重置background_scroll_offset
                
        # 边界处理
        if background_scroll_offset < 0:
            # 超出上边界时柔和地返回
            background_scroll_offset *= 0.8
            # 如果非常接近0，直接设为0
            if background_scroll_offset > -0.5:
                background_scroll_offset = 0
                background_scroll_velocity = 0
        elif background_scroll_offset > max_scroll:
            # 超出下边界时柔和地返回
            over_scroll = background_scroll_offset - max_scroll
            background_scroll_offset = max_scroll + over_scroll * 0.8
            # 如果非常接近最大值，直接设为最大值
            if background_scroll_offset < max_scroll + 0.5:
                background_scroll_offset = max_scroll
                background_scroll_velocity = 0
    
    # 更新目标位置，但只有在明确设置目标位置时才生效
    if background_target_scroll != background_scroll_offset and background_target_scroll >= 0:
        scroll_distance = background_target_scroll - background_scroll_offset
        if abs(scroll_distance) < 0.5:
            background_scroll_offset = background_target_scroll
        else:
            # 更平滑的插值
            background_scroll_offset += scroll_distance * 0.12
...
if __name__ == "__main__":
    try:
        ...
        while running:
            # --- 添加延迟检查逻辑 ---
            # Check if lyrics animation finished and delay passed
            if lyrics_animation_finished and not album_animation_music_started and time.time() >= lyrics_animation_finish_time + 1.0:
                print("歌词动画延迟结束，开始播放音乐。")
                if selected_music_path:
                     play_music(selected_music_path) # 调用播放函数
                     album_animation_music_started = True # 标记音乐已开始
                     # 重置进度条状态，确保从头开始
                     target_progress_width = 0
                     current_progress_width = 0
                     if id3 and album_cover: total_progress_bar_width = album_cover.get_width() # 确保进度条总宽度设置正确

                album_animation_active = False # 结束专辑动画状态
                lyrics_animation_finished = False # 重置标志
                start_ui_animation() # 开始UI元素动画
            # --- 延迟检查结束 ---
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT: # quit program
                    ...
                    if event.key == pygame.K_F3: # toggle debug screen
                        toggle_debug_screen = not toggle_debug_screen
                    elif event.key == pygame.K_ESCAPE and background_settings_active:
                        # ESC键关闭背景设置界面
                        background_settings_active = False
                    elif event.key == pygame.K_SPACE and music_busy: # 空格键控制播放/暂停
                        toggle_play_pause()
                elif event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 1:  # 左键点击
                        if is_mouse_over_settings_btn(event.pos) and not background_animation_active:
                            # 点击设置按钮
                            background_settings_active = not background_settings_active
                        elif background_settings_active:
                            # 在背景设置界面中点击
                            if background_hover_index >= 0:
                                # 点击了某个背景选项
                                selected_bg = background_options[background_hover_index]
                                start_background_animation(selected_bg['path'])
                        elif music_busy and is_mouse_over_progress_bar(event.pos):
                        ...
                        if background_settings_active and mouse_pos[1] > 100:  # 在背景设置界面内
                            # 增加向上滚动的速度
                            background_scroll_velocity -= 12
                            # 限制最大速度
                            background_scroll_velocity = max(background_scroll_velocity, -40)
                            # 重要：将当前位置设为目标位置，防止回弹
                            background_target_scroll = -1  # 使用-1表示不强制设置目标
                            background_scroll_active = True
                        elif not music_busy and music_library:  # 在主页专辑列表
                            # 记录滚动时间和速度
                            current_time = time.time()
                            ...
                            # 设置滚动速度和目标位置
                            scroll_velocity = -mouse_wheel_sensitivity / time_diff
                        ...
                        if background_settings_active and mouse_pos[1] > 100:  # 在背景设置界面内
                            ...
                            background_scroll_velocity += 12
                            # 限制最大速度
                            background_scroll_velocity = min(background_scroll_velocity, 40)
                            # 重要：将当前位置设为目标位置，防止回弹
                            background_target_scroll = -1  # 使用-1表示不强制设置目标
                            background_scroll_active = True
                        elif not music_busy and music_library:  # 在主页专辑列表
